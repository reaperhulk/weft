...
stability. `bayer` is the cheap ordered alternative — a fixed 8x8
threshold matrix, so it compresses better and is more temporally stable
than blue noise, at the cost of visible cross-hatch structure and weaker
banding suppression — and `none` completes the range. Both ordered modes
leave pixels the palette already reproduces exactly untouched, so a
source with 255 or fewer colors encodes losslessly whatever the dither
setting. The mask is generated by
